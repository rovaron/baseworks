...
status: executing
stopped_at: Phase 29 complete (auth/org identity wiring + absorbed `<FileUpload>`; backend live-DB-verified + UI vitest-axe; Docker up)
...
**Current focus:** Phase 29 complete — user avatars + org logos flow through auth's declared `fileRelations` (zero files↔auth import; resolved via `ctx.dispatch`), cascade-on-replace, and the reusable `<FileUpload>` component (absorbed forward from Phase 30) shipped in `packages/ui` + wired into `/profile` + `/team/settings`. Phase 30 (now reduced to the admin tenant-files browser + multi-mode polish) is next.

## Current Position

Milestone: v1.4 File Storage & Uploads
Phase: 29 (identity-assets-fileupload) — COMPLETE (backend live-DB-verified + UI vitest-axe; browser-E2E → 29-HUMAN-UAT; Docker up)
Plan: 1 of 1 (executed from 29-PLAN-CONTRACT.md)
Status: Phase 29 closed — user avatars + org logos flow through auth's declared `fileRelations` ({ user: avatar 64/128/256/512 webp; organization: logo 128/256 webp }), SVG excluded from both allow-lists (rejected at sign-time → `mime_not_allowed` → 400), owner/admin write-gated via auth's OWN `member` table (`isOwnerOrAdmin` — no files import). `get-profile` resolves a SIGNED, expiring `avatarUrl` from the latest user-kind file purely through `ctx.dispatch("files:list-for-record"/"files:get-read-url")` — ZERO `@baseworks/module-files` import (`lint:cross-module` green; only comment mentions remain). Cascade-on-replace: a new `FileRelation.cardinality:"single"` makes `attach-file` (now tx-wrapped) soft-delete every OTHER live row for the owner tuple, refunding `bytes_used` + variant bytes via the EXTRACTED `lib/soft-delete.ts` shared with `delete-file` — no pile-up. The reusable `<FileUpload>`/`useFileUpload` (`packages/ui`, ABSORBED forward from Phase 30) is backend-AGNOSTIC (injected `sign`/`complete`/`onUploaded`; imports neither `@baseworks/api-client` nor `apps/*`): drag-drop + visually-hidden picker fallback, REAL XHR byte progress (`xhr.upload.onprogress`, not synthetic), `URL.createObjectURL` preview (revoked on remove/unmount), cancel (`xhr.abort()`) + retry-from-failed-step, single + `multi` mode, `UploadDescriptor` `kind` discriminator (`s3-put|s3-post|local`), full `UploadErrorCode` set, `beforeunload` guard while uploading, en+pt-BR `files` i18n namespace, vitest-axe a11y. Wired into `/profile` (avatar) + `/team/settings` (org logo, owner-gated) via `GET /api/profile` + the Eden client (sign→PUT→complete→attach→invalidate). avatarUrl is ALWAYS a signed expiring read URL — no raw storage_key/bucket ever surfaced. `bun test packages/modules/files` → 90 pass / 0 fail; auth file-relations + get-profile → 15 pass / 0 fail; `file-upload.a11y.test.tsx` → 8 pass / 0 fail; `build:web` compiles (pre-existing `/signup` prerender bailout aside). Adversarial review: 0 blockers + 2 warnings (self-scoped avatar read via shared handlerCtx + owner-only canRead; cascade refund only on `COUNTED_STATUSES` through shared soft-delete), addressed. Phase 30 reduced to admin tenant-files browser + multi-mode polish. — async image-variant generation wired end-to-end. Spike S-1 GREEN (operator-verified inside `oven/bun:1`: `SHARP_OK bytes=86 fmt=webp w=50 isWebp=true`); the durable gate artifact `sharp/__smoke__/bun-docker-spike.test.ts` is committed so CI/Docker re-run it. IMG-01/IMG-02/IMG-03 satisfied: `SharpImageTransform` (DEFAULT — drops metadata by default = EXIF strip; `limitInputPixels:50M + failOn:'warning'` on every resize) + `ImagescriptImageTransform` (env-selectable fallback — pure JS, encodes webp/jpeg/png, CANNOT decode webp, header-only `metadata()` so the bomb pre-flight never OOMs) both pass ONE shared `runImageTransformConformance` suite (resize + webp magic + EXIF strip + metadata) — sharp 18/18, imagescript 27/27. `file.completed` → enqueue on the `image-transform` BullMQ queue (dynamic `createQueue` import to keep `wrapQueue` out of the hooks import graph) → worker (`concurrency:2`) generates each variant at a DETERMINISTIC key, putObjects it, writes `files.transforms` jsonb + flips `status='ready'` with a SIGNED quota delta (retry-safe). Phase-20 trace propagation spans API→enqueue→worker. 3-layer decompression-bomb defense: (a) image/* >20 MB rejected at `/complete` on the authoritative `stat()` size; (b) sharp pixel+warning guard; (c) worker `metadata()` pre-flight >50M → structured `file.transform-failed` + `status='failed'`, NO throw/crash — PROVEN by the 50000×50000 fixture. Plus a worker source-format allow-list (`png/jpeg/webp/gif`) closing the librsvg SSRF surface, and svg refused at enqueue. EXIF stripped from every variant (EXIF-bearing round-trip gate asserts input HAS GPS+camera markers, output has none). `DATABASE_URL=… REDIS_URL=… bun test packages/modules/files` → 87 pass / 0 fail (313 expects). Sharp RAN on the Windows dev host (win32-x64 prebuilt loaded — not skipped). Adversarial review: 2 blockers (librsvg source-format filter; dynamic `createQueue` import) + 4 warnings (event payload leak → fixed reason code only; retry quota double-count → signed delta; layer-a MIME bypass → gate on `effectiveMime`; sharp host availability → `describe.skipIf` + Docker smoke gate), all addressed.
Next: Phase 30 — Reusable Uploader Component in packages/ui (REDUCED: `<FileUpload>` already shipped in Phase 29; remaining = admin tenant-files browser listing via `list-for-record` + multi-mode polish) (UI-02)
Last activity: 2026-06-17

Progress (v1.4): [███████---] 6 of 8 phases (75%)
...
Stopped at: Phase 29 closed (identity-assets-fileupload; auth `fileRelations` + `avatarUrl` via `ctx.dispatch` + cascade-on-replace + absorbed `<FileUpload>` + `/profile` & `/team/settings`; backend live-DB-verified + UI vitest-axe; Docker up)
Resume file: None
Next action: `/gsd:plan-phase 30` — Reusable Uploader Component (REDUCED scope). The `<FileUpload>`/`useFileUpload` component, the `files` i18n namespace, the vitest-axe a11y test, and the customer-app avatar + org-logo pages already shipped in Phase 29 (UI-01 delivered). Remaining for Phase 30 (UI-02): the ADMIN tenant-files browser in the Vite app — list files via `list-for-record`, consume the already-frozen `<FileUpload>` without duplicating upload logic, multi-mode polish. Also fold in the operator HUMAN-UAT for Phase 29 (`29-HUMAN-UAT.md`: drag→variants, beforeunload, member-403) once the full stack is stood up. Carryover note: the `auth.user-deleted` producer (`{ tenantId, recordId }` cascade payload pinned in Phase 27) — confirm it lands before/within Phase 31 cleanup if not already emitted by Phase 29.
