...
**Plans:** 1/1 complete — executed from `29-PLAN-CONTRACT.md` (LOCKED). Complete. SCOPE: this phase ABSORBED the reusable `<FileUpload>`/`useFileUpload` component from Phase 30 (built to the full Phase 30 component spec). IDA-01/IDA-02 satisfied: auth declares `fileRelations: { user (avatar 64/128/256/512 webp), organization (logo 128/256 webp) }` keyed by `kind`, SVG excluded from both allow-lists (rejected at sign-time → `mime_not_allowed` → 400), owner/admin write-gated via auth's OWN `member` table (`isOwnerOrAdmin`); `get-profile` resolves a signed `avatarUrl` from the latest user-kind file purely through `ctx.dispatch("files:list-for-record"/"files:get-read-url")` with ZERO `@baseworks/module-files` import (`lint:cross-module` green); cascade-on-replace lives in `attach-file` via a new `FileRelation.cardinality:"single"`, sharing `lib/soft-delete.ts` with `delete-file` so a replaced avatar/logo is soft-deleted and `bytes_used` (incl. variant bytes) decrements with no pile-up. `<FileUpload>` (`packages/ui`) is backend-AGNOSTIC (injected `sign`/`complete`/`onUploaded`, no api-client/apps import): drag-drop + picker fallback, real XHR byte progress, `URL.createObjectURL` preview, cancel/retry, single + `multi` mode, `UploadDescriptor` `kind` discriminator (`s3-put|s3-post|local`), all `UploadErrorCode` states, `beforeunload` guard, en+pt-BR `files` i18n namespace, vitest-axe a11y. Wired into `/profile` (avatar) + `/team/settings` (org logo, owner-gated) via `GET /api/profile` + the Eden client. `bun test packages/modules/files` → 90 pass / 0 fail; auth file-relations+get-profile → 15 pass / 0 fail; `file-upload.a11y.test.tsx` → 8 pass / 0 fail; `build:web` compiles (pre-existing `/signup` prerender bailout aside). Adversarial review: 0 blockers + 2 warnings (self-scoped avatar read; cascade refund only on counted statuses via shared soft-delete), addressed. Browser-E2E → `29-HUMAN-UAT.md`.
**UI hint:** yes

#### Phase 30: Reusable Uploader Component in packages/ui

**Goal:** ~~Ship the `<FileUpload>` component + `useFileUpload` hook in `packages/ui`~~ — **DONE in Phase 29** (the component, the `files` i18n namespace, the vitest-axe a11y test, and the `/profile` + `/team/settings` customer pages were all pulled forward into Phase 29). Phase 30 now = **ship the admin tenant-files browser** (Vite app, listing files via `list-for-record`) **+ multi-mode polish** that consumes the already-shipped `<FileUpload>` without duplicating logic.
**Depends on:** Phase 29 (the `<FileUpload>` component API is now frozen there)
**Requirements:** UI-01 (component — DELIVERED in Phase 29), UI-02 (admin browser + cross-app reuse — remaining here)
**Scope change (2026-06-17):** Per operator decision, the reusable `<FileUpload>`/`useFileUpload` component (Phase 30 SC#1, #2, #4, #5) shipped in Phase 29. SC#3's customer-app avatar + org-logo pages also shipped in Phase 29. What remains for Phase 30: the **admin tenant-files browser** half of SC#3.
**Success Criteria** (what must be TRUE):
...
**Plans:** 1/1 complete — executed from `30-PLAN-CONTRACT.md` (LOCKED). Complete (backend fully live-DB-verified + admin UI vitest green; browser-E2E → `30-HUMAN-UAT.md`). UI-02 satisfied: the FULL admin files surface — five cross-tenant admin functions (`adminListFilesForTenant`/`adminSignUpload`/`adminCompleteUpload`/`adminGetReadUrl`/`adminDeleteFile`) in `packages/modules/files/src/commands/admin-files.ts`, plain async fns taking an EXPLICIT `targetTenantId` (NOT `ctx.tenantId`), reusing `reserveQuota`/`markUploaded`/`softDeleteRow`/`buildStorageKey`/`dispositionFor` unchanged and charging the TARGET tenant; a generic `admin-attachment` `fileRelation` (images + pdf, NO svg/gif, 10 MiB, 1 webp thumb, `canRead`/`canWrite=false` for public defense-in-depth) declared on the files module's own `ModuleDefinition` and collected at boot; five routes on `apps/api/src/routes/admin.ts` behind the existing single `.use(requirePlatformAdmin())`, deriving the target tenant from the gated `:id` path param ONLY (body carries no `tenantId`; `kind` fixed server-side; sign verifies tenant existence → 404); and the `apps/admin` tenant-detail **Files** card — `DataTable` list (name/type/size/status+variantCount/created, view → signed read-url `window.open`, delete confirm `Dialog`) + an upload zone consuming the FROZEN Phase-29 `<FileUpload multi>` via new admin sign/complete adapters, with bounded `refetchInterval` so async webp thumbnails surface without reload, en+pt-BR `tenants.detail.files.*` i18n, and the `files` namespace registered in the admin app. `enqueueTransform` was extracted from the `file.completed` subscriber and shared by both the public and admin complete paths (subscriber behaviour unchanged). `storage_key`/`bucket` never in any response; quota charged to the TARGET tenant. `bun test packages/modules/files/.../admin-files.test.ts` → 8 pass / 0 fail; full files module → 98 pass / 0 fail; `apps/api/.../admin-auth.test.ts` (401/403 gate across all 5 endpoints) → 14 pass / 0 fail; `apps/admin` vitest → 27 pass / 0 fail (`detail.files.test.tsx` = 7). Adversarial review: 0 blockers + 3 warnings (publicly-signable global relation → `canRead/canWrite=false` + Phase-31 sweep; key-leak → explicit projection + scan test; async-variant polling → bounded refetch), all addressed.
**UI hint:** yes
