...
- [x] **BILL-01**: Tenant can subscribe to a plan via Stripe Checkout
- [x] **BILL-02**: Tenant can change or cancel subscription
- [x] **BILL-03**: Tenant can make one-time payments for services/products
- [x] **BILL-04**: Usage-based billing tracks consumption per tenant and reports to Stripe
- [x] **BILL-05**: Stripe Customer Portal accessible for self-service billing management
- [x] **BILL-06**: Stripe webhook handler processes events with idempotency (dedup table)
- [x] **BILL-07**: Tenant/user linked to Stripe customer on creation

### Background Jobs

- [x] **JOBS-01**: BullMQ queue infrastructure with Redis connection management
- [x] **JOBS-02**: Each module can register jobs with its own queue and handlers
- [x] **JOBS-03**: Dedicated worker instance mode via `bun run worker` entrypoint
- [x] **JOBS-04**: Transactional email sending (password reset, welcome, billing notifications)
- [x] **JOBS-05**: Stripe webhook events processed via job queue for reliability

### Frontend — Customer App

- [x] **CUST-01**: Next.js app with authentication pages (login, signup, magic link, password reset)
- [x] **CUST-02**: Dashboard layout with sidebar navigation and protected routes
- [x] **CUST-03**: Billing pages (plan selection, subscription status, upgrade/downgrade)
- [x] **CUST-04**: Eden Treaty client wired for type-safe API calls
- [x] **CUST-05**: Tenant context provider and switcher (if user has multiple tenants)

### Frontend — Admin Dashboard

- [x] **ADMN-01**: Vite + React admin app with auth (admin-only access)
- [x] **ADMN-02**: Tenant management panel (list, view, edit, deactivate tenants)
- [x] **ADMN-03**: User management panel (list users, view details, impersonate, ban/activate)
- [x] **ADMN-04**: Billing overview (subscription distribution, revenue, invoices)
- [x] **ADMN-05**: System health panel (BullMQ queue depth, Redis stats, error rates)

### Shared UI

- [x] **SHUI-01**: Shared UI package with shadcn/ui components and Tailwind 4 configuration
- [x] **SHUI-02**: Eden Treaty client package shared across customer and admin apps

### Operations

- [x] **OPS-01**: Dockerfiles for API server, worker process, and admin dashboard
- [x] **OPS-02**: Docker Compose for local development (PostgreSQL, Redis, API, worker, admin)
- [x] **OPS-03**: Health check endpoints for API and worker with dependency status
- [x] **OPS-04**: Structured JSON logging via pino for all backend services
- [x] **OPS-05**: Next.js app configured for Vercel deployment
...
| SHUI-01 | Phase 4 | Complete |
| SHUI-02 | Phase 4 | Complete |
